...
class Analysis {
  ...
  async getDetections(input) {
    ...
    for (let i = 0; i < this.detections.length; i++) {
      let detectionSize = this.detections[i].detection._box._width * this.detections[i].detection._box._height;
    ...
    if (this.detections[closestDetection]) {
      ...
      if (this.faceDimensions.x == null) {
        this.faceDimensions.x = this.detections[closestDetection].detection._box._x;
        this.faceDimensions.y = this.detections[closestDetection].detection._box._y;
        this.faceDimensions.w = this.detections[closestDetection].detection._box._width;
        this.faceDimensions.h = this.detections[closestDetection].detection._box._height;
      } else {
        this.faceDimensions.x +=
          followSpeed * (this.detections[closestDetection].detection._box._x - this.faceDimensions.x);
        this.faceDimensions.y +=
          followSpeed * (this.detections[closestDetection].detection._box._y - this.faceDimensions.y);
        this.faceDimensions.w +=
          followSpeed * (this.detections[closestDetection].detection._box._width - this.faceDimensions.w);
        this.faceDimensions.h +=
          followSpeed * (this.detections[closestDetection].detection._box._height - this.faceDimensions.h);
      }
    } else {
